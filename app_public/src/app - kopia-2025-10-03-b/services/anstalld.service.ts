...
/*
  ...
  */
  /** Hämta en enskild anställd kan behöva ändra 
   * return this.http.get<Anstalld>(`${this.apiUrl}/${id}`) */ 
...
sokAnstalldPaginerat(search: string = '', skip: number = 0, limit: number = 100): Observable<{ anstallda: Anstalld[], total: number }> {
 ...
 /** V1: Sök med paginering och sökterm via query-parametrar */
/*
 sokAnstalldPaginerat(search: string = '', skip: number = 0, limit: number = 100): Observable<{ anstallda: Anstalld[], total: number }> {
  const params = new HttpParams()
    .set('search', search)
...
// version 2 jag har tagit bort denna
/*
