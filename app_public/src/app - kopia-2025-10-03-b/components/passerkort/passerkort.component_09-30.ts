...
*/
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      if (this.idCardCanvas) {
        this.initCanvas(this.idCardCanvas.nativeElement);
      } else {
        console.error('❗ idCardCanvas är inte tillgängligt i ngAfterViewInit');
      }
    }, 0);
  }

/* ny funktion för att skala upp bilden vid utskrift
i webbläsaren är 300×500 px logiskt – alltid samma storlek.
Men under huven har vi gjort canvasen mycket större (t.ex. 600×1000 eller 900×1500 px) beroende på scaleFactor
...
private async initCanvas(canvasEl: HTMLCanvasElement): Promise<void> {
  ...
  //  Sätt intern upplösning (högre än CSS om retina/print ska bli skarpt)
  canvasEl.width = logicalWidth * scaleFactor;
  canvasEl.height = logicalHeight * scaleFactor;

  //  Men visa alltid 300x500 i webbläsaren
  ...
  //  Skala ned koordinatsystemet så att vi jobbar i 300x500
  if (scaleFactor !== 1) {
    this.fabricCanvas.setZoom(scaleFactor);
    this.fabricCanvas.setViewportTransform([scaleFactor, 0, 0, scaleFactor, 0, 0]);
  }
