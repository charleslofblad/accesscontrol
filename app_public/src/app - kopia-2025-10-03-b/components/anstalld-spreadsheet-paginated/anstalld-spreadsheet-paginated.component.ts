...
//private onWindowResize = () => this.resizeColumnsToContainer();
  ...
  initHandsontable(): void {
    ...
    this.hot = new Handsontable(container, {
      ...
      stretchH: 'all',
     // autoColumnSize: false,    // stänger av autosizing baserat på innehåll
    ...
    //  autoWrapRow: false,
     ...
     // kör resize efter en tick — då är DOM & CSS tillämpade och clientWidth är korrekt
/*
  setTimeout(() => {
    this.resizeColumnsToContainer();
    window.addEventListener('resize', this.onWindowResize);
  }, 50);
  */
  // Render efter en tick
  setTimeout(() => this.hot.render(), 50);
  }
/** Beräkna och sätt colWidths så summan inte överstiger container-bredden.
 *  Tar hänsyn till rowHeader (50px default) och en liten buffer för borders.
 */

private resizeColumnsToContainer(): void {
  if (!this.hot || !this.containerRef) return;

  // Säkerställ att Handsontable mäter kolumner efter innehåll
  this.hot.updateSettings({
    autoColumnSize: true,  // låter kolumner breddas efter innehållet
    stretchH: 'none'       // ingen tvingad stretch, behåll naturlig bredd
  });
