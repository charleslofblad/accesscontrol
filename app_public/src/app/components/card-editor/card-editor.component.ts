...
export class CardEditorComponent implements AfterViewInit {
  ...
  /** 1) Initiera canvas version 2
  private initCanvas() {
    const el = this.canvasRef.nativeElement as HTMLCanvasElement;
    el.width = 300; el.height = 500;
    this.canvas = new Canvas(el, { width: 300, height: 500 });
    this.canvas.backgroundColor = '#ffffff';
    this.canvas.calcOffset();
    this.canvas.requestRenderAll();

    // Synka urval med text-redigeringsfält
    ...
    // Ta bort markerat objekt med Delete/Backspace
