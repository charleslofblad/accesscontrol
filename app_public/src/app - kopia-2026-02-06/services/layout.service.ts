...
@Injectable({ providedIn: 'root' })
export class LayoutService {
  private apiUrl = 'http://localhost:8000/api/anstallda/'; // Anpassa till din base URL

  constructor(private http: HttpClient) {}

  saveLayoutToEmployee(id: string, layoutJson: string) {
    return this.http.put(`${this.apiUrl}/${id}`, { Layout: layoutJson });
  }

  getLayoutForEmployee(id: string) {
    return this.http.get<{ Layout: string }>(`${this.apiUrl}/${id}`);
  }
}
*/